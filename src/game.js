SKRABULEC.engine = (function(engine) {
  ...
  var assert = SKRABULEC.utils.assert, gamePrototype = {};

  gamePrototype.register_player_move = function(move) {
    var response = {},
        state, s, i, j, c;

    if (this.is_finished) {
      response.error = engine.string_map.game_finished;
      return response;
    }

    if (move.kind === engine.mnormal) {
      for (i = 0; i < move.tiles.length; i++) {
        if ((move.tiles[i].field = engine.cToi(move.tiles[i].field))
            === undefined) {
          response.error = engine.string_map.move_error;
          return response;
        }
        if (move.tiles[i].letter === undefined) {
          response.error = engine.string_map.set_blank_letter;
          return response;
        }
        move.tiles[i].letter =
      ...
      state = engine.validateMove(this.player_rack,
                                this.current_board,
                                move.tiles);
      ...
      s = engine.supplementRack(this.player_rack, move.tiles, this.bag);
    ...
    if (move.kind === engine.mpause) {
      state = engine.makeState(this.player_rack, engine.mpause);
      state.board = this.current_board.slice(0);
      this.add_to_move_list(state);
      return response;
    }
    if (move.kind === engine.mexchange) {
      if (this.num_player_exchanges >= 3) {
        response.error = engine.string_map.max_three_exchanges;
        return response;
      }
      if (this.bag.size() < 7) {
        response.error = engine.string_map.not_enough_tiles_in_bag;
        return response;
      }
      if (move.tiles.length === 0)
        return response;
      state = engine.makeState(this.player_rack, engine.mexchange);
    ...
    if (move.kind === engine.mresignation) {
      state = engine.makeState(this.player_rack, engine.mresignation);
      state.board = this.current_board.slice(0);
      this.add_to_move_list(state);
  ...
  gamePrototype.get_opponent_move = function() {
    var state, response = {}, s;
    state = engine.generateMove2(this.current_board, this.opponent_rack);
    if (state.move_kind === engine.mnormal) {
      assert(typeof engine.validateMove(this.opponent_rack,
                                      this.current_board,
                                      state.tiles)
             !== "string");
      this.current_board = state.board.slice(0);
      s = engine.supplementRack(this.opponent_rack, state.tiles, this.bag);
      this.opponent_rack = s.rack + s.new_tiles;
      this.add_to_move_list(state);
      response.tiles = state.tiles;
    } else if (state.move_kind === engine.mpause) {
      this.add_to_move_list(state);
      response.pause = true;
    }
    else if (state.move_kind === engine.mexchange) {
      this.add_to_move_list(state);
  ...
  //
  gamePrototype.add_to_move_list = function(state) {
    var i;
    ...
    if (state.move_kind === engine.mnormal) {
      ...
      assert(state.total_points === undefined);
      state.points = 0;
    } else if (state.move_kind === engine.mresignation) {
      assert(state.tiles === undefined);
      assert(state.words === undefined);
      assert(state.board !== undefined);
      assert(state.board.length === 289);
    ...
    if (state.move_kind === engine.mnormal) {
      console.log(engine.getNotation(state.board, state.tiles));
    }
    this.check_game_state();
  };

  gamePrototype.check_game_state = function() {
    ...
    function sum_rack(r) {
      ...
      for (i = 0; i < r.length; i++)
        s += engine.letterMap[r.charAt(i)].npoints;
      return s;
    }
    ...
    function four_consecutive_pauses() {
      ...
      if (mll < 4)
        return false;
      for (i = 1; i <= 4; i++)
        if (that.move_list[mll - i].move_kind !== engine.mpause)
          return false;
      return true;
    }
    ...
    if (r1.length === 0) {
      this.eog_reason = engine.eognormal;
      p1 = sum_rack(r2);
      ...
      this.eog_reason = engine.eogpauses;
      p1 = - sum_rack(r1);
      t1 = p1 + this.move_list[mll - 1].total_points;
      p2 = - sum_rack(r2);
      t2 = p2 + (mll >= 2 ? this.move_list[mll - 2].total_points : 0);
      ...
      this.eog_reason = engine.eogresignation;
  ...
  engine.make_game = function() {
    var game = Object.create(gamePrototype);
    game.bag = this.makeBag();
    game.current_board = this.initBoard();
    game.move_list = [];
